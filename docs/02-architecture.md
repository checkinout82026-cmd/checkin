...
The current app is a browser-only SPA. It does not call an application backend. All persistent reads/writes go directly from the browser to Firestore through the Firebase Web SDK. The data layer mirrors Firestore data into local component state and `localStorage`.
...
- Restores an MVP session from `localStorage.activeUser`.
...
| --- | --- | --- | --- |
| `Login` | Student ID login and staff/admin credential login | `students`, `users` | `activeUser` through parent callback |
| `StudentDashboard` | Self-service check-in/out | `students`, `attendance` | `attendance` |
...
| `activeUser` | MVP persisted session |
...
| `firebase` | Firestore connection and realtime listeners |
